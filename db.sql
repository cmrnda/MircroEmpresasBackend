...
drop table if exists venta_envio cascade;
drop table if exists venta_comprobante cascade;
...
drop table if exists existencia_producto cascade;
...
(
    ...
    foreign key (empresa_id, suscripcion_id) references suscripcion (empresa_id, suscripcion_id) on delete cascade
);
...
(
    ...
    estado                    text           not null,
    confirmado_por_usuario_id bigint references usuario (usuario_id),
    confirmado_en             timestamptz,
    unique (empresa_id, venta_id),
    foreign key (empresa_id, cliente_id) references cliente (empresa_id, cliente_id)
);
...
create index idx_categoria_empresa2 on categoria (empresa_id);
create index idx_producto_empresa2 on producto (empresa_id);
